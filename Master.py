...
TELEGRAM_BOT_TOKEN = '7639626842:AAGQbpq_mfv3qF4d9UQaZdO9xP5iETg7EE4'  # Replace with your bot token
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*❄️ WELCOME TO @MasterBhaiyaa ULTIMATE UDP FLOODER ❄️*\n\n"
        ...
        "*💬 Queries or Issues? 💬*\n"
        "*Contact Admin: @MasterBhaiyaa*"
    )
    await context.bot.send_message(chat_id=chat_id, text=message, parse_mode='Markdown')

async def MasterBhaiyaa(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    args = context.args
    ...
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Tere ko simple command bhi nahi aati? Chal, sikh le: /MasterBhaiyaa <add|rem> <user_id> <coins>*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user["coins"] < COINS_REQUIRED_PER_ATTACK:
        await context.bot.send_message(
            chat_id=chat_id,
            text="*💰 Bhai, tere paas toh coins nahi hai! Pehle admin ke paas ja aur coins le aa. 😂 DM:- @MasterBhaiyaa*",
            parse_mode='Markdown'
        )
        return
...
async def help(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    message = (
        "*🛠️ @MasterBhaiyaa VIP DDOS Bot Help Menu 🛠️*\n\n"
        ...
        "- *Agar koi dikkat aaye toh admin ko contact karo: @MasterBhaiyaa*\n\n"
